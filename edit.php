<?php
/**
 * Student Management System - Update Operation
...
$stmt = $conn->prepare("SELECT id, name, email, phone, birthdate, address, course, year_level, gpa, status 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("UPDATE students SET name = ?, email = ?, phone = ?, birthdate = ?, 
                                address = ?, course = ?, year_level = ?, gpa = ?, status = ? 
                                WHERE id = ?");
        $stmt->bind_param("sssssssdsi", $name, $email, $phone, $birthdate, $address, $course, $year_level, $gpa, $status, $student_id);
...
<body>
    <div class="container">
    <h1>Edit Student</h1>
    <p class="subtitle">Update operation — modify the database record</p>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select id="year_level" name="year_level">
                            <option value="Freshman" <?php echo ($student['year_level'] ?? '') === 'Freshman' ? 'selected' : ''; ?>>Freshman</option>
                            <option value="Sophomore" <?php echo ($student['year_level'] ?? '') === 'Sophomore' ? 'selected' : ''; ?>>Sophomore</option>
                            <option value="Junior" <?php echo ($student['year_level'] ?? '') === 'Junior' ? 'selected' : ''; ?>>Junior</option>
                            <option value="Senior" <?php echo ($student['year_level'] ?? '') === 'Senior' ? 'selected' : ''; ?>>Senior</option>
                            <option value="Graduate" <?php echo ($student['year_level'] ?? '') === 'Graduate' ? 'selected' : ''; ?>>Graduate</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" 
                               id="gpa" 
                               name="gpa" 
                               value="<?php echo $student['gpa'] ? htmlspecialchars($student['gpa']) : ''; ?>" 
                               min="0" 
                               max="4" 
                               step="0.01"
                               placeholder="3.75">
